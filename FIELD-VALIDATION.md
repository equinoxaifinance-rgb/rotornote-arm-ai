...
The current receipt at `field/results/cwru-cross-domain.json` records **0% simulated-envelope coverage**: all four records became `review_required`, zero received automatic conclusions, and all three faulted records produced an unaccepted bearing candidate. The normal record also remained outside the fitted envelope and produced an unaccepted bearing candidate. That is direct evidence that the synthetic envelope does **not** generalize to this external sensor domain. It verifies fail-closed behavior only; representative field data, machine-specific calibration, and qualified review are mandatory before claiming usefulness, sensitivity, specificity, field calibration, or certification. Raw CWRU records are fetched at run time and are not redistributed in this repository.
