...
Two SHA-256-pinned CC BY bearing archives and one public NASA archive produced 100% `review_required` decisions through the canonical one-channel anomaly route. The axial dataset (DOI `10.17632/chwhh9n3bf.2`) contributes 28 records: four healthy captures and 24 seeded-spall captures at two loads and two speeds. The Zhenjiang dataset (DOI `10.17632/xfj9t7cprb.1`) contributes 35 physical traces challenged across all seven documented operating speeds, for 245 boundary executions. NASA IMS adds three natural run-to-failure experiments, 12 bearing installations, 16 sensor trajectories, and 112 predeclared cases; every endpoint channel attached to the four documented failed bearings abstained. Broad FP32/INT8 decisions agreed on every execution. This demonstrates fail-closed behavior on three external sources only, not bearing detection accuracy, natural-fault sensitivity, warning lead time, or field validation.

## Known limitations

Accuracy evidence covers one laboratory rig near 1,238 RPM with experimentally induced conditions. External boundary evidence now includes natural-failure data, but it proves abstention only; it does not establish natural-fault sensitivity. No evidence establishes severity estimation, compound-fault accuracy, sensor interchangeability, changing mounts, broad speed/load ranges, cross-machine accuracy transfer, or field prevalence. Probabilities are classifier scores, not calibrated real-world failure risk. Certification and field reliance require the independent protocol in `FIELD-VALIDATION.md`.

## Variable-speed anomaly head

RotorNote separately ships a 96->609->326->120->8 ReLU MLP whose eight outputs preserve the healthy state and seven observed UPATRAS experimental conditions. Its input is an ordered pair of temporal feature windows from each physical signal. The customer-facing boundary collapses those outputs to `healthy | anomaly`; it never presents a laboratory condition as a field diagnosis. The CC BY 4.0 source (DOI `10.17632/42v3s74gf9.1`) contains 2,925 signals from 39 complete measurement sequences at 75 speeds. Four-fold validation is grouped by complete sequence, with 99.83% observed eight-condition balanced accuracy, 100% broad anomaly balanced accuracy, and 39/39 sequence accuracy (Wilson 95%: 91.0%-100%). Training begins at 96->768->384->128->8; units that never activate anywhere on the complete real training bank are removed before export with 3.82e-6 maximum fitted-bank logit drift. Every exported hidden unit activates on real bank data. This is evidence on that rig, not field certification.

The head uses a 0.90 conservative broad-anomaly score floor, fitted-envelope rejection, and mandatory FP32/INT8 agreement. SIMD-row-padded INT8 reduces learned bytes by 74.22%, preserves every eight-condition production label across the 2,925-signal bank, and has 0.007802 maximum probability drift. It cannot name a field fault family, estimate severity, or replace the four-sensor head.
