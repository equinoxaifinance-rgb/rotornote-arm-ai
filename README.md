...
Three native Arm64 Neoverse-N2 runs over identical FP32, INT8, and WASM hashes measured **3.2164×**, **3.2819×**, and **3.2805× paired-median end-to-end speedups** over FP32. Their deterministic bootstrap 95% intervals were **3.1751×–3.2539×**, **3.2512×–3.3015×**, and **3.2361×–3.3111×** across 51 alternating-order samples per run. Every run rebuilt the repository, passed all 24 tests, and proved exact NEON `vdotq_s32` agreement. These are workload-specific repeatability receipts—not energy, fleet, or universal-device claims. [Inspect run 31693588831](https://github.com/equinoxaifinance-rgb/rotornote-arm-ai/actions/runs/31693588831), [repeat 31693809937](https://github.com/equinoxaifinance-rgb/rotornote-arm-ai/actions/runs/31693809937), and [exact frozen-commit run 31694629861](https://github.com/equinoxaifinance-rgb/rotornote-arm-ai/actions/runs/31694629861).
...
The separate 48->128->64->2 ReLU head uses the CC BY 4.0 [UPATRAS dataset](https://data.mendeley.com/datasets/42v3s74gf9/1): 2,925 real signals across 39 complete measurement sequences and 75 shaft speeds. Four-fold validation holds out whole sequences and observed 100% balanced accuracy plus 39/39 sequence accuracy (Wilson 95%: 91.0%-100%). It answers only `healthy`, `anomaly`, or `review_required`; it never invents a fault family or severity.

FP32 learned bytes are 58,632; row-wise INT8 learned bytes are 15,240, a 74.01% reduction. Across the complete 2,925-signal bank the engines preserve 100% label agreement, with 0.00050 p99 and 0.00586 maximum probability drift. On native Arm64, the deeper path measured a **1.0776x paired-median speedup** with a **1.0742x-1.0798x** deterministic bootstrap 95% interval and 0.000239 maximum benchmark drift. [Inspect exact run 31697964607](https://github.com/equinoxaifinance-rgb/rotornote-arm-ai/actions/runs/31697964607). `POST /api/anomaly?engine=optimized` accepts one uniaxial channel plus sample rate and positive operating RPM. The browser includes one attributed physical demo signal. Its timing is not conflated with the linear head's 3.2x claim.
