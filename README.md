...
- Two separately published CC BY 4.0 bearing rigs attack the canonical one-channel boundary without entering training. RotorNote issued **zero automatic conclusions** on all 28 axial-bearing records and all 245 Zhenjiang RPM challenges derived from 35 physical traces. FP32 and INT8 broad decisions agreed on every challenge. Both publisher archives are SHA-256 pinned and every trace is selected before inference.

The exact receipts are [`field/results/open-grouped-cross-validation.json`](field/results/open-grouped-cross-validation.json), [`field/results/axial-bearing-boundary.json`](field/results/axial-bearing-boundary.json), and [`field/results/zhenjiang-bearing-boundary.json`](field/results/zhenjiang-bearing-boundary.json). Licenses and transformations are in [`DATA-LICENSES.md`](DATA-LICENSES.md).
