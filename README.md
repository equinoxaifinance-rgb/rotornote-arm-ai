...
FP32 learned bytes are 58,632; row-wise INT8 learned bytes are 15,240, a 74.01% reduction. Across the complete 2,925-signal bank the engines preserve 100% label agreement, with 0.00050 p99 and 0.00586 maximum probability drift. `POST /api/anomaly?engine=optimized` accepts one uniaxial channel plus sample rate and positive operating RPM. The browser includes one attributed physical demo signal. This head has its own native benchmark and its timing is not conflated with the linear head's speed claim.
