...
The repository's `ubuntu-24.04-arm` action treats architecture as a hard gate, rebuilds deterministic artifacts, runs all tests, alternates both engines across raw benchmark samples, records machine/dependency details, and uploads hashes. Local x64 receipts remain explicitly separate. Native speed is not claimed until that action produces a real artifact.
